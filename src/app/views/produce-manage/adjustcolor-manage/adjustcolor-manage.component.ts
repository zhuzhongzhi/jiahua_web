  ...
  getProduce()  {
    this.ingotAlarmService.boardOutputToday().subscribe((res) => {
      ...
      res.value.forEach(item => {
        this.doffingWeight += item.colourWeight ? item.colourWeight : 0;
      });
    ...
    if (this.showiFrame) {
      ...
      if (this.isAdd) {
        this.ingotAlarmService.craftAdd(dataInfo).subscribe((res) => {
          this.messageService.showToastMessage('判色记录添加成功', 'success');
          this.detailModal.show = false;
          this.initList();
        });
      } else {
        this.ingotAlarmService.craftModify(dataInfo).subscribe((res) => {
          this.messageService.showToastMessage('判色记录修改成功', 'success');
          this.detailModal.show = false;
          this.initList();
        });
      }
